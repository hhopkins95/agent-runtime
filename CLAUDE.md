...
This is a monorepo for an AI agent runtime system that orchestrates AI agents (Claude via Agent SDK, Gemini via CLI) in isolated Modal sandboxes. It provides a Node.js backend runtime and React client library for building applications with AI agents.
...
- `gemini-cli/` - Adapter for Gemini CLI, parses JSON transcripts into conversation blocks
...
- `AGENT_ARCHITECTURE_TYPE` = `"claude-agent-sdk" | "gemini-cli"`
- `PersistenceAdapter` - Interface for storage backends (sessions, transcripts, workspace files)
- `ConversationBlock` - Unified block format for both agent architectures (user messages, assistant text, tool use/results, thinking, subagents)
- `RuntimeSessionData` - Full session state including parsed blocks
- `SavedSessionData` - Persisted session state with raw transcripts

### Data Flow

1. Client creates session via REST API
2. Runtime creates `AgentSession` which spawns Modal sandbox
3. Client joins WebSocket room for the session
4. Messages sent via REST trigger agent execution in sandbox
5. Agent output streamed as `ConversationBlock` events via WebSocket
...
- Block types support both Claude and Gemini: adapters convert native formats to unified `ConversationBlock`
